    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {classes.length === 0 ? (
        <div className="bg-white p-6 rounded shadow text-sm text-gray-600">
          ...
          <p className="text-xs text-gray-500 mb-4">Nếu bạn là giáo viên, hãy yêu cầu quản trị viên phân công lớp cho bạn.</p>
          <div className="flex items-center gap-2">
            <a href={`mailto:admin@codejudge.local?subject=Y%C3%AAu%20c%E1%BA%A7u%20ph%C3%A2n%20c%C3%B4ng%20l%E1%BB%8Bp%20cho%20${encodeURIComponent('')}`} className="px-3 py-2 bg-indigo-600 text-white rounded">Yêu cầu phân công lớp</a>
            <button onClick={() => window.location.reload()} className="px-3 py-2 border rounded">Tải lại</button>
          </div>
        </div>
      ) : (
